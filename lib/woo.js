...
function normalizeProduct(p) {
  return {
    ...
    slug: p.slug,
    sku: p.sku || null,
    // Conservé pour le tunnel de commande, qui reste sur WooCommerce.
    permalink: p.permalink,
    description: decode((p.short_description || p.description || "").replace(/<[^>]+>/g, " ")).replace(/\s+/g, " ").trim(),
    brand: decode(p.brands?.[0]?.name || ""),
    images: (p.images || []).map((img) => ({
      src: media(img.src),
      alt: decode(img.alt || "") || decode(p.name),
    })),
    categories: (p.categories || []).map((cat) => ({ name: decode(cat.name), slug: cat.slug })),
...
/** Fiche produit par slug. `null` si le produit n'existe plus. */
export async function getProductBySlug(slug) {
  const data = await get(`/products?slug=${encodeURIComponent(slug)}&per_page=1`);
  if (!Array.isArray(data) || !data.length) return null;
  return normalizeProduct(data[0]);
}

/**
 * Tous les slugs du catalogue, pour le sitemap.
 *
 * L'API plafonne à 100 éléments par requête : on pagine jusqu'à épuisement,
 * avec une borne de sécurité pour ne jamais boucler indéfiniment si l'API
 * renvoyait toujours une page pleine.
 */
export async function getAllProductSlugs(maxPages = 10) {
  const slugs = [];

  for (let page = 1; page <= maxPages; page++) {
    const data = await get(`/products?per_page=100&page=${page}&_fields=slug`);
    if (!Array.isArray(data) || !data.length) break;
    slugs.push(...data.map((p) => p.slug).filter(Boolean));
