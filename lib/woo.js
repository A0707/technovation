...
/** Total réel du catalogue, lu dans l'en-tête X-WP-Total. */
export async function getCatalogueSize() {
  try {
    const res = await fetch(`${BASE}/products?per_page=1`, { next: { revalidate: REVALIDATE } });
    if (!res.ok) return null;
    return Number(res.headers.get("x-wp-total")) || null;
  } catch {
    return null;
  }
}
